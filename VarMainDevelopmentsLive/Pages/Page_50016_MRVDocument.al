page 50016 "MRV Document"
{
    Caption = 'PO Document-(Ready For MRV)';
    PageType = Document;
    ...
    SourceTableView = WHERE("Document Type" = FILTER(Order), Status = filter(Released | "Pending Prepayment"));
...
#pragma warning restore
        ...
        area(processing)
        {
            ...
            action(MRV)
            {
                Caption = 'MaterialReceiptVoucher';
                ApplicationArea = all;
                Image = Print;
                ToolTip = 'Executes the MaterialReceiptVoucher action.';
                trigger OnAction()
                begin
                    purchhdr.Reset();
                    purchhdr.SetRange("No.", rec."No.");
                    if purchhdr.FindFirst() then
                        Report.RunModal(REPORT::MaterialReceiptVoucher, true, true, purchhdr);
                end;
            }
            ...
            group("P&osting")
            {
                ...
                action(Post)
                {
                    ...
                    begin
                        /*
                        PurchaseSetup.GET;
                        // PurchaseSetup.TestField(PurchaseSetup.re);
                        NoSeries.Reset();
                        ...
                        Rec.TestField("Receiving No. Series");*/
