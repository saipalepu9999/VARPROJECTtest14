page 50093 "Item Reclassification Journal"
{
        ...
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                field("Location Code"; Rec."Location Code")
                {
                    ...
                    begin
                        LocationGRec.Reset();
                        LocationGRec.SetRange("Item Project Code", true);
                        if LocationGRec.FindSet() then
                            if Page.RunModal(0, LocationGRec) = Action::LookupOK then begin
                                Rec."Location Code" := LocationGRec.Code;
    ...
    begin
        ...
        Rec.ShowShortcutDimCode(ShortcutDimCode);
        Rec.ShowNewShortcutDimCode(NewShortcutDimCode);
        if CopyStr("Item No.", 1, 1) = 'D' then begin
            //Rec.Validate("Location Code", 'DOMSTORES');
            //Rec.Validate("New Location Code", 'DOMSTORES');
            Rec.Validate("Shortcut Dimension 1 Code", 'DOM');
            Rec.Validate("New Shortcut Dimension 1 Code", 'DOM');
        end else
            if CopyStr("Item No.", 1, 1) = 'E' then begin

                //Rec.Validate("Location Code", 'EOUSTORES');
                //Rec.Validate("New Location Code", 'EOUSTORES');
                Rec.Validate("Shortcut Dimension 1 Code", 'EOU');
                Rec.Validate("New Shortcut Dimension 1 Code", 'EOU');
            end;
