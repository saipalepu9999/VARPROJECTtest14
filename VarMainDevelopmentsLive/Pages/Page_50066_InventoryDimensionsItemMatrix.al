page 50066 "Invenotry Dimension Matrix"
{
    Caption = 'Inventory Dimension Wise Matrix';
    Editable = false;
    LinksAllowed = false;
    PageType = ListPart;
    SourceTable = Location;
    //SourceTableView = where("Dimension Code" = const('DIVISION'));
    ...
    begin
        ...
        if TempMatrixLocation.FindSet() then
            repeat
                ...
                MATRIX_OnAfterGetRecord(MATRIX_CurrentColumnOrdinal, Rec.Code);
    ...
    local procedure MATRIX_OnAfterGetRecord(ColumnID: Integer; codeLpar: Code[20])
    var
    ...
    begin
        ...
        //ItemLedgEntryLRec.Copy(Rec);
        //ItemLedgEntryLRec.SetRange("Global Dimension 1 Code", Rec.Code);
        ItemLedgEntryLRec.Reset();
        ItemLedgEntryLRec.SetRange("Location Code", Rec.Code);
        if FacilityCodeGvar <> '' then
            ItemLedgEntryLRec.SetRange("Global Dimension 1 Code", FacilityCodeGvar);
        ItemLedgEntryLRec.SetRange("Global Dimension 2 Code", MatrixRecords[ColumnID].Code);
        ItemLedgEntryLRec.SetRange("Item No.", ItemNoGvar);
        if ItemLedgEntryLRec.FindSet() then begin
            ItemLedgEntryLRec.CalcSums(Quantity);
            MATRIX_CellData[ColumnID] := ItemLedgEntryLRec.Quantity;
        end else
    ...
    procedure Load(MatrixColumns1: array[32] of Text[1024]; var MatrixRecords1: array[32] of Record "Dimension Value"; var MatrixRecord1: Record "Dimension Value"; CurrSetLength: Integer; ItemNoLPar: Code[20]; FacilityCodePar: Code[20])
    begin
        MATRIX_CurrSetLength := CurrSetLength;
        ItemNoGvar := ItemNoLPar;
        FacilityCodeGvar := FacilityCodePar;
    ...
    begin
        ...
        //ItemLedgerEntry.SetRange("Global Dimension 1 Code", Rec.Code);
        ItemLedgerEntry.SetRange("Location Code", rec.Code);
        if FacilityCodeGvar <> '' then
            ItemLedgerEntry.SetRange("Global Dimension 1 Code", FacilityCodeGvar);
        ItemLedgerEntry.SetRange("Global Dimension 2 Code", MatrixRecords[ColumnID].Code);
        ItemLedgerEntry.SetRange("Item No.", ItemNoGvar);
        PAGE.Run(0, ItemLedgerEntry);
    end;
