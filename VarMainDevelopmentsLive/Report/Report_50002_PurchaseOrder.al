report 50002 "Purchase Order Report"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = all;
    DefaultLayout = RDLC;
    //RDLCLayout = 'Report\Layouts\PurchaseOrder - Copy.rdl';
    RDLCLayout = 'Report\Layouts\PurchaseOrder.rdl';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            { }
            column(City; "Buy-from City")
            { }
            column(PostCode; "Buy-from Post Code")
            { }
            ...
            begin
                ...
                if OrderAddressRec.Get("Purchase Header"."Buy-from Vendor No.", "Purchase Header"."Order Address Code") then;

                if "Order Address GST Reg. No." <> '' then
                    GSTRegNo := "Order Address GST Reg. No."
                else
                    GSTRegNo := "Vendor GST Reg. No.";

                if "Order Address Code" <> '' then begin
                    PhoneNo := OrderAddressRec."Phone No.";
                    Email := OrderAddressRec."E-Mail";
                end else begin
                    PhoneNo := VendorGRec."Phone No.";
                    Email := VendorGRec."E-Mail";
                end;
